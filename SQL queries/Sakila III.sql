...
SELECT country, city, COUNT(customer_id) AS total_number_of_customer FROM customer
...
AVG(amount) AS Average_rental_amount FROM payment
...
SELECT DATE_FORMAT(payment_date, '%h %p') AS Month_and_Year, SUM(amount) AS total_payments_received FROM payment
